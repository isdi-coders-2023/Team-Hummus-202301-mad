...
export function AppContextProviderPrivate({ children }: { children: JSX.Element }) {
  const charsRepoPrivate = useMemo(() => new CardApiRepo(), []);
  const context = useCharsPrivate(charsRepoPrivate);

  return <AppContextPrivate.Provider value={context}>{children}</AppContextPrivate.Provider>;
}
